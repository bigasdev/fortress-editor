...
FUtils::get_all_files_in_folder(const std::string &path,
...
std::string FUtils::get_filename_without_extension(const std::string &path) {
  return std::filesystem::path(path).stem().string();
}

bool FUtils::get_file_extension(const std::string &path,
                                const std::string &ext) {
  std::string extension = std::filesystem::path(path).extension().string();
  return extension == ext;
}

bool FUtils::is_corrupted(const std::string &path) { return false; }
bool FUtils::is_empty(const std::string &path) { return false; }
